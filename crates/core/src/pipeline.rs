...
/// Parses lines like "- @mat: Send pricing doc by Friday" into ActionItem structs.
    ...
    summary
        ...
        .map(|item| {
            let (assignee, task) = if let Some(rest) = item.strip_prefix('@') {
                // "@mat: Send pricing doc by Friday"
...
mod tests {
    ...
    fn extract_action_items_parses_assignee_and_task() {
        let summary = summarize::Summary {
            ...
            decisions: vec![],
            action_items: vec![
                "@mat: Send pricing doc by Friday".into(),
    ...
    fn extract_decisions_with_topic_inference() {
        let summary = summarize::Summary {
            ...
            key_points: vec![],
            decisions: vec![
                "Price advisor platform at $399/mo".into(),
        ...
        assert!(decisions[0].text.contains("$399"));
    }
